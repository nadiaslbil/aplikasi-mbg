  ...
  return (
    <AdminLayout currentPage="/dashboard/users" title="Manajemen User" description="Kelola user dan role akses sistem">
      ...
      <div className="filter-bar">
        <button onClick={() => { setEditingId(null); reset({ nama: '', email: '', password: '', role: 'admin_daerah' }); setShowForm(true); }} className="btn-primary">
          <Plus size={16} /> Tambah User
        </button>
        <div className="relative flex-1 max-w-xs ml-auto">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" />
          <input type="text" placeholder="Cari user..." value={search} onChange={(e) => setSearch(e.target.value)} className="input pl-9" />
        </div>
      </div>
              ...
              {loading ? (
                ...
                userList.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td className="hidden md:table-cell text-zinc-500">
                      {new Date(u.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleEdit(u)} className="btn-icon" title="Edit"><Edit size={16} /></button>
                        {u.id !== 1 && (
                          <button onClick={() => handleDelete(u.id)} className="btn-icon-danger" title="Hapus"><Trash2 size={16} /></button>
                        )}
                      </div>
                    </td>
                  </tr>
